...
public class JavaFXEuropeanChessDisplay implements JavaFXChessBoardDisplay {
  ...
  public void displayEmptySquare(Button button, int row, int column) {
    if ((row + column) % 2 == 0) {
      button.setBackground(new Background(new BackgroundFill(blackColor, CornerRadii.EMPTY, Insets.EMPTY)));
    } else {
      button.setBackground(new Background(new BackgroundFill(redColor, CornerRadii.EMPTY, Insets.EMPTY)));
    }
    button.setText(null);
  }
  ...
  public void displayFilledSquare(Button button, int row, int column, ChessPiece piece) {
    ...
    // array of color for cell background and color of piece
    BackgroundFill[] backgroundFillArray = new BackgroundFill[]{button.getBackground().getFills().get(0), new BackgroundFill(pieceColor, new CornerRadii(cornerRad), new Insets(3.0))};
    ...
    if (highlight) {
      ...
      if (piece != null) {
        ...
        BackgroundFill[] backgroundFillArray = new BackgroundFill[]{new BackgroundFill(highlightColor,
                                                                                       CornerRadii.EMPTY, Insets.EMPTY), new BackgroundFill(pieceColor,
                                                                                                                                               new CornerRadii(cornerRad), new Insets(3.0))};
        // highlight the cell
        button.setBackground(new Background(backgroundFillArray));
